...
# Inputs:  
- `u`: array of `3N` positions followed by `3N` velocities
  `[x_1, y_1, z_1, x_2, y_2, z_2, ...., vx_1, vy_1, vz_1, ..., vz_N]`
- `mass`: array of N masses
- `t`: time

# Outputs:
- `du`: Aray of `3N` velocities followed by `3N` accelerations
...
function gravity_as_first_order_system(du, u, mass, t)
    ...
    for i in 0:(N-2)      # loop over each pairs of bodies
        for j in (i+1):(N-1)
            ...
            one_over_d_cubed = 1/sqrt(dx^2+dy^2+dz^2)^3
            # derivatives of velocities are accelerations 
            # acceleration on body i due to body j
            du[3N+3*i+1] -= mass[j+1] * dx * one_over_d_cubed
            du[3N+3*i+2] -= mass[j+1] * dy * one_over_d_cubed
            du[3N+3*i+3] -= mass[j+1] * dz * one_over_d_cubed
            # reuse computation of d for acceleration on body j due to body i
            du[3N+3*j+1] += mass[i+1] * dx * one_over_d_cubed
            du[3N+3*j+2] += mass[i+1] * dy * one_over_d_cubed
            du[3N+3*j+3] += mass[i+1] * dz * one_over_d_cubed
        end
    end
end

# ╔═╡ 05f2bcdf-9ec0-43b9-9d94-5a8592e80ce1
protip(md"""We could have simplified the algorithm slightly by having both `i` and `j` loop over each planet.  Instead, the limits of each loop are set so that we only consider each pair once.  The most expensive part of calculating the acceleration is the `sqrt` function.  Thus, we speed up the calculation by about a factor of two.""", invite="Click to exapnd if you're curious about the loop ranges?")
...
md"""
Rather than writing our own code to solve differential equations, we'll make use of the [DifferentialEquations.jl package](https://docs.sciml.ai/DiffEqDocs/stable/).  It's big, powerful and complex.  Here we'll use just a few of it's features.  Below I illustrate how to create an "ODEProblem" object by providing the function to calculate derivatives (`gravity_as_first_order_system`), the initial conditions (`u_init`), a "Tuple" (here used to store an ordered-pair) with the start and stop time, and a list of fixed parameters (here the masses of the bodies, `m`).  
"""
...
response_3a = missing # INSERT your response as a String or Markdown 

# ╔═╡ 1660f55e-6834-48f1-a9c0-f3108faadd93
display_msg_if_fail(check_type_isa(:response_3a,response_3a,[AbstractString,Markdown.MD])) 
...
md"""
That's not the most intuitive way to visualize the results.  Below, I'll create a scatter plot and use `idxs=(10,11)` to specify that we want the 10th and 11th variables (x and y position of body 2), so we can see the trajectory of the planet more clearly.  I've added some additional [optional arguements](http://docs.juliaplots.org/latest/attributes/) to make the plot a little prettier.
"""

# ╔═╡ 1d08ed26-aee4-4760-ac3b-fa1166c4189e
scatter(sol_a,idxs=(10,11), markersize = 0, xlabel="x (AU)", ylabel="y (AU)",
...
response_3b = missing; # md"INSERT RESPONSE"

# ╔═╡ f1db3d6b-7216-412b-a596-d3492a3fdac6
display_msg_if_fail(check_type_isa(:response_3b,response_3b,[AbstractString,Markdown.MD])) 
...
scatter(sol_c,idxs=(10,11),markersize = 0, xlabel="x (AU)", ylabel="y (AU)", 
...
response_3c = missing; # md"INSERT RESPONSE"

# ╔═╡ 70371442-6d13-4f9d-b609-0934151dc83b
display_msg_if_fail(check_type_isa(:response_3c,response_3c,[AbstractString,Markdown.MD])) 
...
response_3d = missing; # md"INSERT RESPONSE"

# ╔═╡ ba74243b-4cee-4b4e-bf21-9ece6923a3ef
display_msg_if_fail(check_type_isa(:response_3d,response_3d,[AbstractString,Markdown.MD])) 
...
response_3e = missing; # md"INSERT RESPONSE"

# ╔═╡ 0c79007e-9721-45eb-9c2d-5a603dd7efaa
display_msg_if_fail(check_type_isa(:response_3e,response_3e,[AbstractString,Markdown.MD])) 
...
response_3f = missing; # md"INSERT RESPONSE"

# ╔═╡ c518c2a0-c053-4451-aec1-8e89128600f1
display_msg_if_fail(check_type_isa(:response_3f,response_3f,[AbstractString,Markdown.MD])) 
...
"""
   `calc_phase_error(sol; phase_init = 0.0, year = 2π, pl_id=1)`
...
Inputs:
   ...
   - `year`: time required for the phase to increase by 2π
   - `pl_id`: index of the planet to calculate the orbital phase of
Outputs:
...
Calculate the z-component of the angular momentum of the system"
...
function calc_angular_momentum(u::Vector, m::Vector)
    ...
    # Positions are stored in i=[1,2].  Velocities are in i[3,4]
	u2d = reshape(u,3,4)
    L  = m[1] * cross(u2d[:,1], u2d[:,3])
    L += m[2] * cross(u2d[:,2], u2d[:,4])
    #L   # full angular momentum vector
    L[3] # Return just the z-component
end
...
response_3g =  missing; # md"INSERT RESPONSE"

# ╔═╡ a5507391-1d49-4709-bc9d-bcf79f2c1ca3
display_msg_if_fail(check_type_isa(:response_3g,response_3g,[AbstractString,Markdown.MD])) 
...
response_3h =  missing; # md"INSERT RESPONSE"

# ╔═╡ af16d7e3-1b71-4c7d-a126-48c32122fa7e
display_msg_if_fail(check_type_isa(:response_3h,response_3h,[AbstractString,Markdown.MD])) 
...
# Inputs:  
- `v`: array of 3N velocities
- `u`: array of 3N positions 
  `[x_1, y_1, z_1, x_2, y_2, z_2, ...., z_N]`
- `mass`: array of N masses
- `t`: time

# Outputs:
- `du`: Aray of 3N position derivatives
"""
...
# Outputs:
- `dv`: Aray of 3N velocity derivatives
...
# Optional names arguements: (default value)
- `alg`: algorithm to use (KahanLi6(); see [manual]( http://docs.juliadiffeq.org/stable/solvers/dynamical_solve.html#Symplectic-Integrators-1))
- `duration`: duration to integrate in orbits (100)
...
response_3k =  missing; # md"INSERT RESPONSE"
...
response_3l =  missing; # md"INSERT RESPONSE"

# ╔═╡ 675d7d77-8efb-4519-872f-50003c555610
display_msg_if_fail(check_type_isa(:response_3l,response_3l,[AbstractString,Markdown.MD])) 
...
response_3m = missing; # md"INSERT RESPONSE"

# ╔═╡ 137014c4-7da5-4bed-937f-0b33ace84be7
display_msg_if_fail(check_type_isa(:response_3m,response_3m,[AbstractString,Markdown.MD])) 
...
[[deps.LinearSolve]]
deps = ["ArrayInterface", "ChainRulesCore", "ConcreteStructs", "DocStringExtensions", "EnumX", "GPUArraysCore", "InteractiveUtils", "Krylov", "LazyArrays", "Libdl", "LinearAlgebra", "MKL_jll", "Markdown", "PrecompileTools", "Preferences", "RecursiveArrayTools", "Reexport", "SciMLBase", "SciMLLogging", "SciMLOperators", "Setfield", "StaticArraysCore", "UnPack"]
git-tree-sha1 = "cf918e72417d878cf4f92c7cde4cbdcc8b97a0ee"
